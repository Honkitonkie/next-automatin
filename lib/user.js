...
export async function getAccessToken(body) {
  ...
  if (email) {
    let db = await dbConnect();
    let thisURI = `https://www.linkedin.com/oauth/v2/accessToken?grant_type=authorization_code&client_id=${process.env.LINKEDIN_CLIENT_ID}&client_secret=${process.env.LINKEDIN_CLIENT_SECRET}&redirect_uri=${process.env.LINKEDIN_REDIRECT_URI}&code=${body.query.code}`;
    // lvl1
    const res = axios
      .post(thisURI)
      .then((res) => {
