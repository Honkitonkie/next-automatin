  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      User.findOne({ email: email })
        .then((user) => {
          if (user) {
            console.log("Mailadres al bekend");
            reject("Mailadres al bekend");
          } else if (!email.includes("@") || email.includes(" AND ")) {
            reject("Email klopt niet");
          } else if (company === "" || company === "Bedrijfsnaam invoeren") {
            reject("Bedrijf is verplicht");
          } else if (username === "") {
            reject("Kies een gebruikersnaam");
          } else if (username === "") {
            reject("Kies een gebruikersnaam");
          } else {
        ...
        .catch((err) => {
          console.log("catch err @ signup: ", err);
          reject("Niks gebeurt!");
        });
    } catch (e) {
...
export async function updateUserFeedType(body) {
  ...
  return User.findOneAndUpdate({ email: body.email }, { upsert: true })
    .then((user) => {
      switch (body.feedTypeSelector) {
        case "person":
          user.linkedin.feed_type.person = true;
          user.linkedin.feed_type.organization = false;
          break;
        case "organization":
          user.linkedin.feed_type.person = false;
          user.linkedin.feed_type.organization = true;
          break;
        case "both":
          user.linkedin.feed_type.person = true;
          user.linkedin.feed_type.organization = true;
...
export async function updateFacebookAccessToken() {
  return { FACEBOOK_CLIENT_ID: process.env.FACEBOOK_APP_ID, FACEBOOK_REDIRECT_URI: process.env.FACEBOOK_REDIRECT_URI };
}

export async function getFacebookAccessToken(body) {
  const email = body.state;
  let thisURI = `https://graph.facebook.com/v15.0/oauth/access_token?client_id=${process.env.FACEBOOK_APP_ID}&redirect_uri=${process.env.FACEBOOK_REDIRECT_URI}&client_secret=${process.env.FACEBOOK_CLIENT_SECRET}&code=${body.code}`;
  // lvl1
  const res = axios
    .post(thisURI)
    .then((res) => {
      ...
      return User.findOne(query).then((user) => {
        ...
        user.facebook.access_token = user.facebook.access_token ? accesToken : accesToken;
        user.facebook.expires_in = user.facebook.expires_in ? expires_in : expires_in;
        user.facebook.token_expire_date = user.facebook.token_expire_date
          ? new Date(Date.now() + expires_in * 1000).toLocaleString().split(",")[0]
          : new Date(Date.now() + expires_in * 1000).toLocaleString().split(",")[0];
        user.facebook.creationDate = user.facebook.creationDate ? Date.now() : Date.now();
        user.facebook.token_type = token_type;

        user.facebook.automatin_client_secret = user.facebook.automatin_client_secret ? user.facebook.automatin_client_secret : crypto.randomBytes(16).toString("hex");
        user.facebook.automatin_client_id = user.facebook.automatin_client_id ? user.facebook.automatin_client_id : process.env.AUTOMATIN_CLIENT_ID;
        user.facebook.templates = user.facebook.templates ? user.facebook.templates : User().facebook.templates;
        user.save();
      });
    })
    .catch((err) => console.log("err @getaccestoken FB: \n", err.message));
}

export async function getFacebookTest(body) {
  return User.findOne({ email: body }).then((user) => {
    // console.log(user.facebook.access_token);

    // let getMeData = `https://graph.facebook.com/v15.0/me/accounts?access_token=${user.facebook.access_token}`;
    // const res = axios
    //   .get(getMeData)
    //   .then((res) => {
    //     console.log("res.getMeData", res.data);
    //     user.facebook.name = res.data.name;
    //     user.facebook.id = res.data.id;
    //   })
    //   .catch((err) => console.log("err @getMeData FB: \n", err.message));

    let postdata = `https://graph.facebook.com/v15.0/me/accounts?access_token=${user.facebook.access_token}`;
    const ress = axios
      .get(postdata)
      .then((res) => {
        console.log("res.postdata", res.data);
        // console.log("res.postdata", res.data.data[0].id);
        const name = res.data.data[0].name;
        const id = res.data.data[0].id;

        let getPageData = `https://graph.facebook.com/v15.0/${id}?fields=pages&access_token=${user.facebook.access_token}`;
        const ress = axios
          .get(getPageData)
          .then((res) => {
            console.log("data", res.data);
          })
          .catch((err) => console.log("err @some tokens FB: \n", err.message));
      })
      .catch((err) => console.log("err @postdata FB: \n", err.message));

    // let getPageData = `https://graph.facebook.com/me/groups?access_token=${user.facebook.access_token}`;
    // const ress = axios
    //   .get(getPageData)
    //   .then((res) => {
    //     console.log("body", res.body);
    //     console.log("data", res.data);
    //     console.log("data.category", res.data.data[0]);
    //     console.log("data.category_list", res.data.data.category_list);
    //   })
    //   .catch((err) => console.log("err @getaccestoken FB: \n", err.message));

    //   let instaPost = `https://graph.facebook.com/v15.0/17841400008460056/media
    // ?image_url=https//www.example.com/images/bronz-fonz.jpg
    // &caption=%23BronzFonz?access_token=${user.facebook.access_token}`;
    //   const ress = axios
    //     .post(instaPost)
    //     .then((res) => {
    //       console.log("body", res.body);
    ...
    //   let getPageData = `https://graph.facebook.com/PAGE-ID?
    // fields=${access_token}&
    // access_token=${}`;
    //   let res = axios
    //     .get(getPageData)
...
export async function removeFacebookAccessToken(email) {
  console.log("body removeFacebookAccessToken", email);
  if (email) {
    // let db = await dbConnect();
    const query = { email: email };
    return User.findOne(query).then((user) => {
      ...
      user.linkedin.expires_in = "";
      user.linkedin.token_expire_date = "";
      user.save();
    });
  }
}

export async function updateLinkedinAccessToken() {
  return { LINKEDIN_CLIENT_ID: process.env.LINKEDIN_CLIENT_ID, LINKEDIN_REDIRECT_URI: process.env.LINKEDIN_REDIRECT_URI };
}

export async function getLinkedinAccessToken(body) {
  const email = body.state;
...
export async function removeLinkedinAccessToken(email) {
  console.log("body removeLinkedinAccessToken", email);
  if (email) {
    // let db = await dbConnect();
    const query = { email: email };
    return User.findOne(query).then((user) => {
...
export async function updateUserAddComment(body) {
  // console.log("update made");
  User.findOne({ email: body.email }).then((user) => {
    if (user) {
      user.linkedin.addComment = body.addComment;
      user.linkedin.commentIntro = body.commentIntro;
      user.save();
      return user;
    } else {
      return "401";
