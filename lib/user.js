...
// Here you should lookup for the user in your DB
export async function updateUser({ email, feedType, templateVal, index }) {
  let db = await dbConnect();
  ...
  return User.findOneAndUpdate({ email: email })
    .then((user) => {
      ...
      if (!unChanged) {
        ...
        // console.log("No Update required!", email, feedType, templateVal, index);
      }
    })
    .catch((err) => console.log("err @ updateuser:", err));
}
// Here you should lookup for the user in your DB
export async function updateSettings({ body }) {
  console.log("updateSettings running");
  let db = await dbConnect();
